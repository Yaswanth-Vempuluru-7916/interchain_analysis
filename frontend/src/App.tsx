...
const App = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [averagesResponse, ordersResponse, anomalyOrdersResponse] = await Promise.all([
        axios.post<AveragesResponse>('http://localhost:3000/averages', {
          start_time: startTime ? startTime.toISOString() : undefined,
          end_time: endTime ? endTime.toISOString() : undefined,
        }),
        axios.post<OrdersResponse>('http://localhost:3000/orders/all', {
          start_time: startTime ? startTime.toISOString() : undefined,
          end_time: endTime ? endTime.toISOString() : undefined,
        }),
  ...
  //   try {
  //     await axios.post('http://xcgg04skw4k044ws8swok4gw.65.109.18.60.sslip.io/sync');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#E0F5F5] to-[#A7E4E0] p-8 text-gray-800 font-sans">
      ...
      <div id="portal">
        ...
        <div className="max-w-7xl mx-auto mb-12">
          <h2 className="text-2xl font-bold mb-6 inline-block border-b-2 border-[#F06292] pb-2 text-gray-800">Bitcoin Anomaly Thresholds (Mean + 2SD)</h2>
          {isFetching ? (
